...
describe("getTransaction", () => {
  ...
  it("Succedd to get transaction", () => {
    ...
    .then(userData => {
      userId = userData.id
      return data.insertTransaction(new TransactionData(null, userData.id, "XRPUSDT", "BUY", 1000, 0.1, 1000*0.1, "2026-04-24T00:00:00Z", 1000))
    })
  ...
  it("Fails on non-existing user", () => {
    ...
    .then(() => data.findTransactionsBySymbol(userId, "XRPUSDT"))
    .then(([txData]) => logic.getTransaction("111111112222222233333333", txData.id))
  ...
  it("Fails on existing user and existing transaction from different user", () => {
    ...
    return Promise.all([
      data.insertUser(new UserData(null, "Annie", "admin@example.com", "annie123", hashed, "http://www.image.com/1", "regular")),
      data.insertUser(new UserData(null, "Fernando", "info@example.com", "fer123", hashed, "http://www.image.com/1", "regular"))
    ])
    .then(() => Promise.all([
    ...
    .then(([user1, user2]) => {
      userId1 = user1.id
      userId2 = user2.id
      return data.insertTransaction(new TransactionData(null, user1.id, "XRPUSDT", "BUY", 1000, 0.1, 1000*0.1, "2026-04-24T00:00:00Z", 1000))
    })
  ...
  it("Fails on existing user and non existing transaction", () => {
    ...
    return data.insertUser(new UserData(null, "Annie", "admin@example.com", "annie123", hashed, "http://www.image.com/1", "regular"))
    .then(() => data.findUserByEmail("admin@example.com"))
    .then(userData => logic.getTransaction(userData.id, "111111112222222233333333"))
    .catch(error => caught = error)
    .finally(() => {
      expect(caught).to.be.instanceOf(ExistenceError)
