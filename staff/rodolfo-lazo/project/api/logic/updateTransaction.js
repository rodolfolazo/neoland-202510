...
export function updateTransaction(
  ...
  return data
    ...
    .then((results) => {
      ...
      if (results.length === 1) {
        transactionsOldSymbol = transactionsOldSymbol.filter(
          (tx) => tx.id !== transactionsOldSymbol.id,
        )
    ...
    .then(() => {
      const arrBalance = []
      arrBalance.push(
        calculateBalanceChain(userId, oldTransaction.symbol, executedAt),
      )

      if (transactionsNewSymbol.length >= 1) {
        arrBalance.push(
          calculateBalanceChain(userId, newTransaction.symbol, executedAt),
        )
      }
