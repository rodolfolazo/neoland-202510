...
describe("logic", () => {
  ...
  describe("authenticateUser", () =>{
    it('Authenticate a user succesfully', () =>{
      return data.insertUser(new UserData(null, 'Annie', 'admin@example.com', 'annie', hashed, "http://www.google.com", "regular"))
    ...
    it('Fails on non-existing user', () =>{
      let caught = null
      return logic.authenticateUser('annie','123123123')
      .catch( error => caught = error)
      .finally(() =>{
  ...
  describe("getUser", () =>{
    ...
    it('Fails non existing user', () =>{
      let caught = null
      return logic.getUser('111111111122222222223333')
      .catch(error =>{
        caught = error
      })
      .finally(() =>{
  ...
  describe("addTransaction", () => {
    it("Add transaction succesfully", () => {
      ...
      .then(()=>data.findUserByUsername("annie"))
      .then(userData => {
        return logic.addTransaction(userData.id, "ETHUSDT", "BUY", 0.5, 2145.32, "2026-05-09")
    ...
    it("Fails when there is not enough balance", () =>{
      ...
      .then(userData => {
        return logic.addTransaction(userData.id, "ETHUSDT", "SELL", 0.5, 2145.32, "2026-05-09")
      })
      .catch(error => caught = error)
      .finally( () =>{
        expect(caught).to.be.instanceOf(ValidationError)
        expect(caught.message).to.equal("not enough balance at this point in history")
      })
    })
  })
